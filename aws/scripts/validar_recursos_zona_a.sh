...
security_group_id=$(aws ec2 describe-security-groups --group-names "app-horadoqa" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)
if [ $? -eq 0 ]; then
  echo "[OK] Security Group app-horadoqa foi criado"
  
  # Validar inbound rule para o security group 'app-horadoqa'
  ...
  # Validar outbound rule para o security group 'app-horadoqa'
  ...
  echo ">[ERRO] Não achei o security group app-horadoqa. Ele foi criado?"
fi
